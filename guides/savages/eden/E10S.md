...
+ *Chest n°2: Edenmorn Head/Hand/Foot coffer + Crypt Dusting (Accessory Upgrade)*
...
*Shadowkeeper* will cast Umbra Slash on the main target, use your invulnerability and swap tank after the fourth hit. Then either stack with the rest of the group if **Darkness Unleashed** is cast or mitigate alone if **Shadow's Edge** is cast.
...
*Shadowkeeper* will cast **Spawn Shadow**, **Shadow Warrior** and **Umbral Orbs** in succession.
...
The boss will then cast **Deepshadow Nova** into **Fade to Shadow**. After a few seconds, **Implosion** will be cast, reducing the number of **Fade to Shadow** safe spots from 2 to 1. After the first set of **Fade to Shadow** happens, move to the closest adjacent inter-cardinal to avoid the second set.
...
Once the gates are resolved, another set of gates will appear. And another **Shadow Servant** will be cast, targeting the four players that weren't targeted before.
...
**Pitch Bog** will then be cast. Drop the puddles along the edges.
...
Afterward, two **Implosions** will be cast followed by a **Deepshadow Nova**.
...
Then,**Fade to Shadow** will be cast with **Umbral Orbs**. Two safe spots will be designated on opposite inter-cardinals.  
...
Instantly move away to avoid getting cleave by your shadow. Right after, **Shadowkeeper** will be cast, move in front of your assigned tower so that your shadow will teleport behind you, inside the tower. Don't greed this and put it properly.
